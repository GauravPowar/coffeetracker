...
export async function onRequestGet({ request, env }) {
  try {
    ...
    // ── ICB Bean Search Proxy ────────────────────────────
    if (route === "icb") {
      const reqUrl = new URL(request.url);
      const q      = reqUrl.searchParams.get("q") || "";
      const slug   = reqUrl.searchParams.get("slug") || "";
      const key    = (env as any).ICB_API_KEY || "";
      if (!key) return json({ error: "ICB_API_KEY not configured" }, 500);
    ...
    if (route === "journal") {
      ...
      await db.prepare(
        `INSERT OR REPLACE INTO journal
         (id, date, brewer, bean_id, bean_label, dose, yield, time, temp, grind, notes, rating, tastes, updated_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`
      ).bind(
        String(j.id), j.date || "", j.brewer || "", j.beanId || "", j.beanLabel || "",
...
export async function onRequestPut({ request, env }) {
  try {
    ...
    if (route === "journal") {
      const j = body;
      await db.prepare(
        `UPDATE journal SET date=?,brewer=?,bean_id=?,bean_label=?,dose=?,yield=?,time=?,temp=?,
         grind=?,notes=?,rating=?,tastes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`
      ).bind(
        j.date || "", j.brewer || "", j.beanId || "", j.beanLabel || "",
        j.dose || 0, j.yield || 0, j.time || 0, j.temp || 0,
        j.grind || "", j.notes || "", j.rating || 0,
